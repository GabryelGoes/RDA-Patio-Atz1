...
const getStageColors = (stage: Stage) => {
  ...
  if (s.includes('não aprovado') || s.includes('nao aprovado')) return 'bg-purple-600 text-white border-purple-500';
...
const VehicleRow: React.FC<VehicleRowProps> = ({ vehicle }) => {
  ...
  const isDelayed = vehicle.deliveryDate === 'ATRASADO';

  return (
    <div className={`flex items-center w-full h-full rounded-[24px] border px-8 shadow-xl ${colorClass}`}>
      <div className="w-[22%]">
        <h2 className="text-3xl font-black tracking-tighter truncate uppercase italic leading-none">{vehicle.model}</h2>
      ...
      <div className="w-[16%] border-l border-current/10 pl-6">
        <p className="text-xl font-bold truncate uppercase">{vehicle.client}</p>
      </div>
      <div className="w-[34%] border-l border-current/10 pl-6">
        <div className="flex items-center gap-2">
          ...
          {s.includes('aprovado') && !s.includes('não') && <span className="text-3xl animate-bounce">✓</span>}
          {s.includes('não aprovado') && <span className="text-3xl animate-pulse">✕</span>}
          <p className="text-3xl font-black uppercase italic tracking-tighter">{vehicle.stage}</p>
        </div>
      </div>
      <div className="w-[14%] border-l border-current/10 pl-6">
        <p className={`font-black uppercase tracking-tighter ${isDelayed ? 'text-4xl animate-pulse text-white' : 'text-2xl'}`}>
          {vehicle.deliveryDate}
        </p>
      </div>
      <div className="w-[14%] border-l border-current/10 pl-6">
        <p className="text-xl font-bold uppercase truncate">{vehicle.mechanic}</p>
      </div>
    </div>
  );
};
